...
## 2026-08-23 - Fix : CSS absente en réel + menu mobile/bandeau cookies inertes

Deux bugs distincts remontés par le client en testant en conditions réelles (mobile + navigateur classique).

**1) CSS absente (page rendue sans aucun style, police par défaut du navigateur)** : un `public/hot` orphelin (créé par un `npm run dev` lancé par un sous-agent puis jamais proprement arrêté) faisait pointer `@vite` vers un serveur de dev Vite (`localhost:5173`) qui ne tournait plus. Supprimé + `npm run build` relancé. Fichier `public/hot` correctement ignoré par Git (`.gitignore`), donc rien à committer sur ce point - juste un nettoyage d'environnement local. **Point de vigilance pour la suite : ne jamais lancer `npm run dev` en tâche de fond sans le stopper explicitement (`Ctrl+C` propre), sinon `public/hot` reste et casse le rendu en environnement `artisan serve`.**

**2) Menu hamburger mobile et bandeau de consentement cookies "inertes" (clic sans effet apparent)** : diagnostiqué en détail via clics JS simulés + vérification de l'état réactif Alpine (`Alpine.$data()`) avant/après clic, avec un vrai délai entre écriture et lecture (le premier essai sans délai donnait un faux négatif - piège classique, le flush des effets Alpine est asynchrone). Résultat : la donnée réactive (`mobileOpen`) se mettait bien à jour, mais l'attribut `x-transition` **nu** (sans classes `x-transition:enter/leave` explicites) sur `<x-public-navbar>` restait bloqué indéfiniment en état caché (`display:none`) après une bascule, empêchant le panneau de s'afficher malgré un clic parfaitement fonctionnel. Le bandeau cookies, qui utilise lui la forme **explicite** (`x-transition:enter="..." x-transition:enter-start="..." x-transition:enter-end="..."`), fonctionne correctement (vérifié avec délai) - seule la forme raccourcie posait problème avec cette version d'Alpine embarquée par Livewire (3.15.12). Recherche exhaustive (`grep`) : un seul usage de la forme nue dans tout le projet, corrigé en `x-show` simple sans transition. Suite de tests toujours 31/31 après correction.

## 2026-08-23 - Remplacement des images placeholder par de vraies photos trading/forex

Le client a signalé que les images `picsum.photos` (photos aléatoires génériques) ne faisaient pas référence au trading. Demande initiale : scraper Pinterest - refusée car le bouton "télécharger" de Pinterest ne donne aucune licence de réutilisation commerciale (la plupart des pins sont des photos protégées reposées sans autorisation), ce qui exposerait le client à un risque de contrefaçon sur son site public. Proposé et validé par défaut : Unsplash (licence explicite d'usage commercial libre, sans attribution requise).

**Action** : recherche Unsplash ("forex trading", "stock trading desk"), 20 photos téléchargées dans `public/images/trading/trading-01.jpg` à `trading-20.jpg` (candlestick charts, postes de trading multi-écrans, laptop/mobile). Script ponctuel (`scratch_replace_images.php`, supprimé après exécution) : remplacement automatique de toutes les URLs `picsum.photos/seed/...` par un mapping déterministe (hash CRC32 du seed → 1 des 20 images), pour que chaque section garde toujours la même image de manière stable. 58 remplacements automatiques + 7 cas particuliers corrigés à la main (seeds construits dynamiquement avec un `$item->id` ou une catégorie d'instrument, non capturés par le regex).

**Point de vigilance traité** : 3 usages détournaient une image de graphique trading en avatar circulaire de témoignage client (`rounded-full`, 40-56px) - visuellement incohérent (une photo de chandelier japonais recadrée en photo de "profil"). Remplacés par un nouveau composant `<x-avatar-initials>` (cercle CSS avec initiales du nom, dégradé couleur-principale/secondaire), cohérent avec la philosophie du projet (logo déjà construit en CSS/HTML plutôt qu'en image).

**Vérification** : 0 référence `picsum.photos` restante (`grep` exhaustif), `php artisan view:cache` réussi (aucune erreur de syntaxe Blade sur les 24 pages), suite de tests toujours 31/31, images servies en 200 (`image/jpeg`) via `/images/trading/*.jpg`.

## 2026-08-23 - Refonte complète du système Wallet (dépôts manuels, statuts, solde démo)

Revue demandée avant de dessiner le parcours de test utilisateur complet (inscription → dépôt → trade → clôture).

**Constat initial** : `WalletTransactionService` (pending → validation admin → crédit `solde_reel`) et `<x-status-badge>` (badges colorés dans l'historique) étaient déjà corrects, construits par le sous-agent Auth+Client. Le vrai manque : le formulaire de dépôt ne montrait jamais au client où envoyer ses fonds (juste un select + un montant), et un seul moyen de paiement crypto générique existait (3 méthodes au total, dont une "Carte bancaire" à tort qualifiée d'instantanée alors qu'aucune passerelle n'est branchée).

**Modèle `payment_methods`** : ajout de la colonne `details_paiement` (texte), distincte du champ `instructions` (explication générale) - contient l'adresse crypto / email PayPal / identifiant Perfect Money / IBAN réellement affiché au client. `PaymentMethodSeeder` entièrement revu : 8 moyens actifs (BTC, ETH, USDT TRC20, USDT ERC20, LTC, PayPal, Perfect Money, Virement bancaire), tous en traitement **manuel** - "Carte bancaire" supprimée. Formulaire Filament `PaymentMethodResource` mis à jour en conséquence (select pour `type`, nouveau champ `details_paiement`).

**`DepositForm` (Livewire)** : `wire:model` passé en `.live` sur le select de moyen de paiement pour afficher dynamiquement, dès la sélection, un bloc `<x-accordion>` repliable (ouvert par défaut, pour ne pas noyer le popup avec des instructions parfois longues) contenant : le texte d'instructions, un champ `<input readonly>` avec bouton copier (adapté : libellé "Adresse de dépôt" + QR code généré via `api.qrserver.com` uniquement si `type === 'crypto'`, sinon libellé générique "Détails du dépôt" sans QR), et le délai de traitement. Popup élargi (`max-width="sm"` → `"md"`) pour laisser respirer ce contenu.

**Solde démo librement rechargeable** : nouveau composant `App\Livewire\Client\TopUpDemoForm` + popup dédié sur la page Wallet (bouton discret superposé à la stat-card "Solde démo"). Contrairement au dépôt réel, incrémente `solde_demo` **instantanément**, sans passer par `WalletTransaction`/validation admin (argent fictif, purement pour laisser le client tester librement - conforme à la demande "il doit pouvoir choisir à volonté son solde démo"). Boutons de montants prédéfinis (+1000/+5000/+10000/+50000) en plus de la saisie libre.

**Vérification end-to-end en navigateur réel** (compte `test@example.com`) : dépôt PayPal de $250 → transaction créée en `en_attente` (solde réel inchangé, badge ambre "En attente") → approbation via `WalletTransactionService::approve()` (simulant l'admin) → solde réel crédité à $250.00, badge passé en vert "Validé". Recharge démo testée avec le préréglage +$5000 : solde démo $10 000 → $15 000 instantanément. Adresse BTC + QR code confirmés corrects pour un moyen crypto ; QR absent et libellé générique confirmés pour PayPal. Suite de tests toujours 31/31. Données de test nettoyées après vérification.
